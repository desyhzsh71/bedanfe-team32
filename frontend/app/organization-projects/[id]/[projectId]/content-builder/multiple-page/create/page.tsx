...
import {
    Sun, Moon, ChevronDown, LogOut, CreditCard, Settings, ArrowLeft, Layout,
} from 'lucide-react';
...
function ToggleRow({ label, desc, value, onChange, darkMode }: {
    label: string; desc: string; value: boolean; onChange: (v: boolean) => void; darkMode: boolean;
}) {
    return (
        <div className="flex gap-4">
            <div className="flex-1">
                <p className="text-sm font-medium mb-0.5" style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>{label}</p>
...
export default function CreateMultiplePage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const token = getToken()!;
            const res = await api.createMultiplePage(projectId, {
                name, apiId, multiLanguage, seoEnabled: seo, workflowEnabled: workflow,
            }, token);
            if (res.data?.id) {
                router.push(`/organization-projects/${orgId}/${projectId}/content-builder/multiple-page/${res.data.id}`);
            } else {
                goBack();
            }
        } catch (e: any) {
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-xs mb-1" style={{ color: darkMode ? '#64748B' : '#94A3B8' }}>
                                    Project / Content Builder / Create Multiple Page
                                </p>
                                <h2 className="text-2xl font-bold" style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>Create Multiple Page</h2>
                            </div>
                        </div>
                    ...
                    <div className="w-full max-w-lg">
                        <div className="rounded-2xl shadow-sm border"
                            ...
                            <div className="flex items-center gap-3 px-7 pt-7 pb-5">
                                ...
                                <div>
                                    <h3 className="text-lg font-bold" style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>New Multiple Page</h3>
                                    <p className="text-xs" style={{ color: darkMode ? '#64748B' : '#94A3B8' }}>Fill in the details below to create your page</p>
                                </div>
                            </div>
                            ...
                            <div className="flex px-7 gap-6 border-b" style={{ borderColor: darkMode ? '#3F3F52' : '#E2E8F0' }}>
                                {(['basic', 'advanced'] as const).map(t => (
                                    <button key={t} onClick={() => setTab(t)}
                                        ...
                                        style={{ borderColor: tab === t ? COLORS.primary : 'transparent', color: tab === t ? COLORS.primary : darkMode ? '#94A3B8' : '#64748B' }}>
                                        {t === 'basic' ? 'Basic Configuration' : 'Advanced Configuration'}
                                    </button>
                                ))}
                            </div>
                            ...
                            <div className="px-7 py-6">
                                {tab === 'basic' ? (
                                    <div className="space-y-5">
                                        <div>
                                            <label className="block text-sm font-medium mb-1.5" style={{ color: darkMode ? '#E0E0E0' : '#374151' }}>Page Name</label>
                                            <input type="text" value={name} onChange={e => handleNameChange(e.target.value)} placeholder="e.g. Blog Posts"
                                    ...
                                    <div className="space-y-5">
                                        <ToggleRow label="Multi Language" darkMode={darkMode}
                                            desc="Enable to make the page support multiple languages."
                                            value={multiLanguage} onChange={setMultiLanguage} />
                                        <ToggleRow label="SEO" darkMode={darkMode}
                                            desc="Enable SEO settings to optimize metadata for search engines."
                                            value={seo} onChange={setSeo} />
                                        <ToggleRow label="Workflow" darkMode={darkMode}
                                            desc="Enable automated workflows with a structured approval process."
                                            value={workflow} onChange={setWorkflow} />
                                    </div>
                                )}
