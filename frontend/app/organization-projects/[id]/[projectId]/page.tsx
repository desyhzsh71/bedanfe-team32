...
export default function ProjectDetailPage() {
    ...
    const [showDuplicateToPersonalModal, setShowDuplicateToPersonalModal] = useState(false);
    const [duplicatingToPersonal, setDuplicatingToPersonal] = useState(false);
    ...
    const handleDuplicateProject = async () => {
        try {
            ...
            const response = await api.duplicateProject(projectId, token);
    ...
    const handleDuplicateToPersonal = async () => {
        try {
            setDuplicatingToPersonal(true);
            const token = getToken();
            if (!token) return;

            const response = await api.duplicateProjectToPersonal(projectId, token);

            if (response.success) {
                setShowDuplicateToPersonalModal(false);
                alert('Project duplicated to personal successfully!');
                router.push('/personal-projects');
            }
        } catch (error: any) {
            console.error('Error duplicating to personal:', error);
            alert(error.message || 'Failed to duplicate project to personal');
        } finally {
            setDuplicatingToPersonal(false);
        }
    };
    ...
    if (!project) {
        return (
            <div className="min-h-screen flex items-center justify-center"
                style={{ backgroundColor: darkMode ? '#1E1E2E' : '#F5F7FA' }}>
                <p style={{ color: darkMode ? '#E0E0E0' : '#64748B' }}>Project not found</p>
            </div>
        );
    }
            ...
            <div className="flex-1 flex flex-col">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-xs mb-1" style={{ color: darkMode ? '#64748B' : '#94A3B8' }}>
                                    Pages / Organization Projects / {orgName} / {project.name}
                                </p>
                ...
                <div className="flex-1 p-8 overflow-y-auto">
                        ...
                        <div className="flex justify-between items-start">
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="px-5 py-2.5 text-white font-medium rounded-lg transition flex items-center gap-2"
                                ...
                                <tr className="border-t transition-colors duration-200"
                                    ...
                                    <td className="px-6 py-4 font-medium w-1/3"
                                        style={{
                                            ...
                                            backgroundColor: darkMode ? 'rgba(63, 63, 82, 0.4)' : '#F1F5F9',
                                        }}>
                                        Project ID
                                    </td>
                                ...
                                <tr className="border-t transition-colors duration-200"
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex justify-between items-center">
                                            <span style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>{project.name}</span>
                                            <button
                                                onClick={() => setShowEditModal(true)}
                    ...
                    <div className="rounded-xl p-6 mb-6 transition-all duration-300"
                        style={{
                            backgroundColor: darkMode ? 'rgba(254, 243, 199, 0.08)' : '#FEF3C7',
                            border: `1px solid ${darkMode ? '#3F3F52' : '#E2E8F0'}`,
                            borderLeft: `4px solid ${colors.warning}`,
                        }}>
                        <p className="text-sm mb-4" style={{ color: darkMode ? '#94A3B8' : '#64748B' }}>
                            By default, your site on CMS can be reached through a subdomain based on your project name. To make it more personalized, add your own custom domain
                        </p>
                        <div className="rounded-lg p-4 flex items-center justify-between mb-4"
                            style={{
                                backgroundColor: darkMode ? '#2D2D3F' : '#FFFFFF',
                                border: `1px solid ${darkMode ? '#3F3F52' : '#E2E8F0'}`,
                            }}>
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-medium" style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>
                        ...
                        <button
                            onClick={() => setShowEditModal(true)}
                            className="w-full px-4 py-2.5 text-gray-900 font-medium rounded-lg transition text-sm"
                    ...
                    <div className="rounded-xl p-6 mb-6 transition-all duration-300"
                        style={{
                            backgroundColor: darkMode ? 'rgba(58, 122, 195, 0.08)' : '#EFF6FF',
                            border: `1px solid ${darkMode ? '#3F3F52' : '#E2E8F0'}`,
                        ...
                        <div className="flex items-start justify-between">
                            <div>
                                ...
                                <p className="text-sm mb-2" style={{ color: darkMode ? '#94A3B8' : '#64748B' }}>
                                    Manage approval stages and content workflow
                                </p>
                                <p className="text-sm font-medium" style={{ color: colors.info }}>
                                    Setup workflow stages for your content publishing process
                                </p>
                            </div>
                            <button
                                onClick={() => router.push(`/organization-projects/${orgId}/${projectId}/workflow-approval`)}
                                className="px-5 py-2.5 text-white font-medium rounded-lg transition flex items-center gap-2 whitespace-nowrap"
                                style={{ backgroundColor: colors.info }}
                                onMouseEnter={(e) => { e.currentTarget.style.opacity = '0.9'; }}
                                onMouseLeave={(e) => { e.currentTarget.style.opacity = '1'; }}>
                                <Workflow size={18} />
                    ...
                    <div className="rounded-xl p-6 transition-all duration-300"
                        style={{
                            backgroundColor: darkMode ? 'rgba(249, 50, 50, 0.08)' : '#FEF2F2',
                            border: `1px solid ${darkMode ? '#3F3F52' : '#E2E8F0'}`,
                        ...
                        <div className="flex items-start justify-between mb-5 pt-5"
                            style={{ borderTop: `1px solid ${darkMode ? '#3F3F52' : '#FECACA'}` }}>
                            <div>
                                <h4 className="font-semibold mb-1"
                                    style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>
                                    Duplicate to Personal
                                ...
                                <p className="text-sm" style={{ color: darkMode ? '#94A3B8' : '#64748B' }}>
                                    You are about to transfer this project to another section. The original project will remain unchanged, while a duplicate will be created in the selected section.
                                </p>
                            </div>
                            <button
                                onClick={() => setShowDuplicateToPersonalModal(true)}
                                className="p-2 rounded-lg transition"
                                style={{ color: colors.error }}
                                onMouseEnter={(e) => {
                                    e.currentTarget.style.backgroundColor = darkMode ? 'rgba(249, 50, 50, 0.1)' : '#FEE2E2';
                    ...
                    <div className="rounded-xl shadow-2xl w-full max-w-md transition-all duration-300"
                        ...
                        <div className="flex gap-3 p-6 border-t"
                            ...
                            <button
                                ...
                                style={{ backgroundColor: colors.secondary }}
                                onMouseEnter={(e) => {
                                    if (!updating && editName.trim()) e.currentTarget.style.opacity = '0.9';
                                }}
                                onMouseLeave={(e) => { e.currentTarget.style.opacity = '1'; }}>
            ...
            {showDuplicateModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="rounded-xl shadow-2xl w-full max-w-md transition-all duration-300"
                        ...
                        <div className="p-6">
                            <p className="mb-3" style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>
                        ...
                        <div className="flex gap-3 p-6 border-t"
                            ...
                            <button
                                ...
                                onMouseEnter={(e) => {
                                    if (!duplicating) e.currentTarget.style.opacity = '0.9';
                                }}
                                onMouseLeave={(e) => { e.currentTarget.style.opacity = '1'; }}>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="rounded-xl shadow-2xl w-full max-w-md transition-all duration-300"
                        ...
                        <div className="flex justify-between items-center p-6 border-b"
                            ...
                            <button
                                onClick={() => setShowDuplicateToPersonalModal(false)}
                                className="p-2 rounded-lg transition"
                                style={{
                                    backgroundColor: darkMode ? '#3F3F52' : '#F1F5F9',
                                    color: darkMode ? '#E0E0E0' : '#1E293B',
                        ...
                        <div className="p-6">
                            <p className="mb-3" style={{ color: darkMode ? '#E0E0E0' : '#1E293B' }}>
                                Are you sure you want to duplicate "{project.name}" to your personal projects? A new copy will be created.
                            </p>
                            <p className="text-sm" style={{ color: darkMode ? '#94A3B8' : '#64748B' }}>
                                The original project will remain in your organization.
                            </p>
                        </div>
                        <div className="flex gap-3 p-6 border-t"
                            style={{ borderColor: darkMode ? '#3F3F52' : '#E2E8F0' }}>
                            ...
                            <button
                                ...
                                disabled={duplicatingToPersonal}
                                className="flex-1 px-4 py-3 text-white font-medium rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed"
                                style={{ backgroundColor: colors.info }}
                                onMouseEnter={(e) => {
                                    if (!duplicatingToPersonal) e.currentTarget.style.opacity = '0.9';
                                }}
                                onMouseLeave={(e) => { e.currentTarget.style.opacity = '1'; }}>
                                {duplicatingToPersonal ? 'Duplicating...' : 'Duplicate'}
                            </button>
                        </div>
                    </div>
                </div>
